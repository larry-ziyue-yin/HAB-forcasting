...
- Per-lake (Great Lakes) time series with coverage
...
# -----------------------------
# 2) Per-lake time series (Great Lakes)
# -----------------------------
def _lake_name_map(df):
    """Resolve a friendly lake name column."""
    for c in ["lake_name", "Lake_name"]:
        if c in df.columns:
            return c
    return None
...
def plot_lake_facets_daily():
    ...
    df["date"] = pd.to_datetime(df["date"])
    name_col = _lake_name_map(df)
    # Pick five target lakes by name if available
    ...
    if name_col:
        ...
        for nm in desired:
            ...
            if not subset.empty:
                lids = subset["lake_id"].unique().tolist()
                order.extend(lids)
...
def plot_validity_heatmap_daily():
    ...
    # Pivot to matrix lakes x dates with values {0,1}
    # Missing → NaN (plotted as white)
    mat = df.pivot_table(index="lake_id", columns="date", values="qc_is_valid", aggfunc="max")
    # Sort lakes by name if available
    if id2name:
    ...
    # Downsample x ticks to avoid clutter
